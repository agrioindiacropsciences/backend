...
export const adminCreateProductSchema = z.object({
  ...
  pack_sizes: z.preprocess(parseJsonArray, z.array(z.object({
    size: z.string().min(1, 'Pack size is required'),
    sku: z.string().optional(),
    mrp: z.coerce.number().min(0).optional(),
    selling_price: z.coerce.number().min(0).optional(),
  })).min(1, 'At least one pack size with rate is required')).refine(
    (arr) => arr.every((ps) => (ps.mrp != null && ps.mrp > 0) || (ps.selling_price != null && ps.selling_price > 0)),
    { message: 'Each pack size must have MRP or Selling price (rate)' }
  ),
